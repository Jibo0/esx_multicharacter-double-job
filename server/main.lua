...
local function SetupCharacters(source)
    ...
    local result = MySQL.query.await("SELECT identifier, accounts, job, job_grade, job2, job_grade2, firstname, lastname, dateofbirth, sex, skin, disabled FROM users WHERE identifier LIKE ? LIMIT ?", { identifier, slots })
    local characters
    ...
    if result then
        ...
        for i = 1, characterCount, 1 do
            ...
            local job2, grade2 = v.job2 or "unemployed", tostring(v.job_grade2)
            ...
            if ESX.Jobs[job2] and ESX.Jobs[job2].grades[grade2] then
                if job ~= 'unemployed' then 
                    grade2 = ESX.Jobs[job2].grades[grade2].label 
                else 
                    grade2 = '' 
                end
                job2 = ESX.Jobs[job2].label
            end

            --[[if job == 'unemployed' then 
                gradoDaMostrare = ' - Disoccupato' 
                print('[Grado Lavoro 1]'.. gradoDaMostrare)
            else 
                gradoDaMostrare = " - " .. ESX.Jobs[job].grades[tonumber(grade)].label 
                print('[Grado Lavoro 1]'.. gradoDaMostrare)
            end
            
            if job2 == 'unemployed' then 
                gradoDaMostrare2 = ' - Disoccupato' 
                print('[Grado Lavoro 2]'.. gradoDaMostrare2)
            else 
                gradoDaMostrare2 = " - " .. ESX.Jobs[job2].grades[tonumber(grade2)].label 
                print('[Grado Lavoro 2]'.. gradoDaMostrare2)
            end]]
